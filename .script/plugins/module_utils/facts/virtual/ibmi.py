...
class IBMiVirtual(Virtual):
    ...
    def get_virtual_facts(self):
        if ibmi_util.HAS_ITOOLKIT:
            ...
            rc, out, error = ibmi_util.itoolkit_run_sql(connection, sql)
            system_values = {}
            for item in out:
            ...
            rc, out, error = ibmi_util.itoolkit_run_sql(connection, "SELECT * FROM QSYS2.SYSCATALOGS;")
            virtual_facts['OS400_RDBDIRE'] = out
            rc, out, error = ibmi_util.itoolkit_run_sql(connection, "SELECT * FROM QSYS2.SYSTEM_STATUS_INFO;")
            virtual_facts['OS400_SYSTEM_STATUS_INFO'] = out
            rc, out, error = ibmi_util.itoolkit_run_sql(connection, "SELECT * FROM QSYS2.NETSTAT_INTERFACE_INFO;")
            virtual_facts['OS400_TCPIP_INFO'] = out
            rc, out, error = ibmi_util.itoolkit_run_sql(connection, "SELECT * FROM QSYS2.GROUP_PTF_INFO;")
            virtual_facts['OS400_GROUP_PTF_INFO'] = out
            rc, out, error = ibmi_util.itoolkit_run_sql(connection, "SELECT CAST(data as VARCHAR(100)) FROM QUSRSYS.QATOCTCPIP WHERE KEYWORD='RMTNAMESV'")
