...
class ActionModule(ActionBase):
    ...
    def run(self, tmp=None, task_vars=None):
        ...
        try:
            ...
            display.debug("Start to copy script file to remote. src = %s." % (src))
            ...
            if remote_checksum in ('1', '2', None):
                result['stderr'] += "The permissions are lacking or privilege is needed. remote_checksum:%s" % remote_checksum
            elif local_checksum != remote_checksum:
                result['stderr'] += "local_checksum doesn't match remote_checksum. remote_checksum = %s, local_checksum = %s." \
            ...
            if result['rc']:
                result['failed'] = True
                result.update(dict(stderr=("Failed to execute script file %s." % src) + result['stderr'], delta=str(delta)))
            else:
                result.update(dict(stdout="Successfully execute script file %s." % src, delta=str(delta)))

        except Exception as e:
            result['stderr'] += "%s" % to_text(e)
            result['failed'] = True
            return result
        finally:
