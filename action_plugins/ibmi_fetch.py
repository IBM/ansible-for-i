...
class ActionModule(ActionBase):
    ...
    def _calculate_savf_path(self, object_names, lib_name):
        ...
        if lib_name != 'QSYS':
            savf_path = '/QSYS.LIB/%s.LIB/%s.FILE' % (lib_name, object_names)
        else:
            savf_path = '/QSYS.LIB/%s.FILE' % (object_names)
    ...
    def _calculate_savf_name(self, object_names, lib_name, is_lib, savefile_name, task_vars, result):
        ...
        if savefile_name is None:
            ...
            while (self._execute_remote_stat(savf_path, all_vars=task_vars, follow=False))['exists']:
                if i > 9:
                    result['msg'] = 'SAVF names (%s range(1,9)) are already exist on IBMi. Failed' % (savf_path)
                    result['failed'] = True
                    return result
        ...
        try:
            ...
            if ifs_created is True:
                ...
                try:
                    ...
                    result['msg'] += "exception happens when delete IFS file. error: %s" % to_text(e)
