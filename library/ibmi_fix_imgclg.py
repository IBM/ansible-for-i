...
executed_commands:
    description: The executed CL commands in this task.
    type: list
    returned: When error occurs.
    sample: [
...
import time
import shutil
from ansible.module_utils.ibmi.temp_directory import TemporaryDirectory
...
def install_by_image_catalog(module, product_id_list, virtual_image_list, dir_target,
    ...
    if (virtual_image_list is not None) and (virtual_image_list != ["*ALL"]):
        command_map["cl_crt_catalog"] = "CRTIMGCLG IMGCLG(" + catalog_name_to_create + ") DIR('" + dir_target +\
    ...
    command_map["cl_inst_ptf"] = "INSPTF LICPGM((" + product_id_str + ")) DEV(" + opt_device_name\
    ...
    command_map["cl_delete_image_catalog"] = "DLTIMGCLG IMGCLG(" + catalog_name_to_create + ") KEEP(*YES)"
    ...
    if (virtual_image_list is not None) and (virtual_image_list != ["*ALL"]):
        for image_name in virtual_image_list:
            cl_catalog_entry_adding = "ADDIMGCLGE IMGCLG(" + catalog_name_to_create + ") FROMFILE("\
...
def main():
    ...
    if use_temp_path:
            ...
            if os.path.isdir(tmp_dir):
                if (fix_file_name_list == ["*ALL"]) or (fix_file_name_list is None):
                    ...
                    for fix_file_name in fix_file_name_list:
                        if os.path.isfile(fix_file_name):
                            source_file = os.path.join(path, fix_file_name)
                            shutil.copy(source_file, tmp_dir)

                rc, out, err = install_by_image_catalog(module, product_id, None, tmp_dir,
        ...
        if os.path.exists(path) is False:
            module.fail_json(msg="The src directory does not exist.")
        if os.path.isdir(path) is False:
            module.fail_json(msg="The value specified in src is not a valid directory.")
        rc, out, err = install_by_image_catalog(module, product_id, fix_file_name_list, path,
