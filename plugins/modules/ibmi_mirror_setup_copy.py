...
def get_location_code_from_resource_name(resource_name):
    conn = dbi.connect()
        ...
        if int(rhrl0100_t['rhrNbr']) > 0:
            ...
            for rec in res_t:
                if rec['resKnd'] == kKindVirtEthernet or rec['resKnd'] == kKindPhysEthernet:
                    ibmi_util.log_debug("resource name is " + rec['resNam'], 'list_rdma_location_code')
                    rc, cur_loc, result = get_location_code_from_resource_name(rec['resNam'])
                    ibmi_util.log_debug("location code is " + cur_loc, 'list_rdma_location_code')
                    ibmi_util.log_debug("get location code rc is {0}".format(rc), 'list_rdma_location_code')
                    ibmi_util.log_debug("get location code result is " + result, 'list_rdma_location_code')
...
def main():
    ...
    if rc or ("INFO Save to file /QIBM/UserData/QDB2MIR/MRDB/TOOLS/meta_data" not in out):
        module.fail_json(rc=rc, msg="Error occurred when generate system snapshot", stdout=out, stderr=err)
    ...
    if (json_file is None) or (not os.path.exists(json_file)):
        module.fail_json(rc=255, msg="Mirror configuration meta data not found")

    # Load the meta_data.json as dict
    json_dict = load_meta_data(json_file)
    ibmi_util.log_debug("Current meta data: {0}".format(str(json_dict)), module._name)
    ...
    if rc:
        module.fail_json(rc=255, msg="Unable retrieve the secondary node information from QSYS2.MIRROR_INFO", job_log=job_log)
    ...
    ibmi_util.log_debug("Secondary system name and hostname in QSYS2.MIRROR_INFO is: {0}, {1}".format(secondary_system_name, sencodary_host_name), module._name)

    # Prepare the RDMA address
    sql = "SELECT TARGET_ADDRESS FROM QSYS2.NRG_LINK_INFO WHERE NRG_NAME = 'MIRROR_DATABASE'"
    rc, out, err, job_log = ibmi_util.itoolkit_run_sql_once(sql)
    if rc:
        module.fail_json(rc=255, msg="Unable retrieve NRG link information from QSYS2.NRG_LINK_INFO", job_log=job_log)
    target_address = []
    for item in out:
        target_address.append(item['TARGET_ADDRESS'])
    # Remove the duplicate
    target_address = list(set(target_address))
    ibmi_util.log_debug("Target address in QSYS2.NRG_LINK_INFO is: {0}".format(target_address), module._name)

    rc, location, result = list_rdma_location_code()
    if rc:
        module.fail_json(rc=rc, msg="Error when list RDMA location code", job_log=result)
    ibmi_util.log_debug("List RDMA resources location result is: {0}".format(result), module._name)
    # Remove the duplicate
    location = list(set(location))
    ibmi_util.log_debug("RDMA resources location are: {0}".format(location), module._name)
    ...
    ibmi_util.log_debug("Already configured IP addresses are: {0}".format(already_configured_ip_list), module._name)
    ibmi_util.log_debug("Already configured cmnlocation are: {0}".format(already_configured_cmn_list), module._name)

    for item in already_configured_ip_list:
        if item in target_address:
            target_address.remove(item)
    ibmi_util.log_debug("Non-configured IP addresses are: {0}".format(target_address), module._name)
    for item in already_configured_cmn_list:
        if item in location:
            location.remove(item)
    ibmi_util.log_debug("Non-configured cmnlocation are: {0}".format(location), module._name)
    ...
    if rdma_subnet_mask == '*SAME':
        sql = "SELECT SUBNET_MASK from QSYS2.NETSTAT_INTERFACE_INFO \
            ...
            ip_address)
        ibmi_util.log_debug("Retrieve RDMA subnet mask sql is: {0}".format(sql), module._name)
        rc, out, err, job_log = ibmi_util.itoolkit_run_sql_once(sql)
        if rc:
            module.fail_json(rc=255, msg="Error occured when retrieving subnet mask information from QSYS2.NETSTAT_INTERFACE_INFO", job_log=job_log)
        else:
            if isinstance(out, list) and len(out) == 1:
                rdma_subnet_mask = out[0]['SUBNET_MASK']
            else:
                module.fail_json(rc=255, msg="Row incorrect when retrieving subnet mask information from QSYS2.NETSTAT_INTERFACE_INFO", stdout=out)
    ibmi_util.log_debug("RDMA subnet mask is: {0}".format(rdma_subnet_mask), module._name)
    ...
    ibmi_util.log_debug("Write meta data to seed: {0}".format(str(json_dict)), module._name)
    save_meta_data(CLOUDINIT_METADATA_DIR + "/meta_data.json", json_dict)
    save_meta_data(DB2MTOOL_METADATA_DIR + "/meta_data_" + secondary_system_name + "_ansible.json", json_dict)
    ...
    if rc and ('ACTIVATION ENGINE OR CLOUD-INIT IS ALREADY ENABLED' not in err):
        module.fail_json(rc=255, msg="Error occurred when enable Activation Engine", stdout=out, stderr=err)
