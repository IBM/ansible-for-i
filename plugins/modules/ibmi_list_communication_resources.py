...
DOCUMENTATION = r'''
---
module: ibmi_list_communication_resources
short_description: Retrieves all the communication resources information in the system.
version_added: '2.8'
description:
  - The C(ibmi_list_communication_resources) module list the communication resources information of the system.
options:
  dummy:
    description:
...
resource_info:
    description: the resources information
...
job_log:
    ...
    returned: always
    type: list
    sample: "{u'dftccsid': u'37', u'jobnbr': u'029163', u'xmlhint': u'01567:7905:Table ANSIBLE_T2 in QGPL created but was not jour',
    u'jobuser': u'QUSER', u'usrlibl': u'QGPL QTEMP QDEVELOP QBLDSYS QBLDSYSR',
    u'jobname': u'QSQSRVR', u'curuser': u'CHANGLE', u'version': u'XML Toolkit 2.0.2-dev', u'xmlhint1':
    u'01567:7905:Table ANSIBLE_T2 in QGPL created but was not jour', u'syslibl': u'QSYS QSYS2 QHLPSYS QUSRSYS',
    u'jobipcskey': u'FFFFFFFF', u'paseccsid': u'0', u'ccsid': u'37'}"
'''
...
def get_info_from_resource_name(resource_name):
    conn = dbi.connect()
        ...
        if int(rhrl0100_t['rhrNbr']) > 0:
            ...
            for rec in res_t:
                if rec['resKnd'] == kKindVirtEthernet or rec['resKnd'] == kKindPhysEthernet:
                    ibmi_util.log_debug("resource name is " + rec['resNam'], sys._getframe().f_code.co_name)
                    rc, res_info, result = get_info_from_resource_name(rec['resNam'])
                    ibmi_util.log_debug("resource info is " + str(res_info), sys._getframe().f_code.co_name)
...
def main():
    ...
    rc, resoure_info, result = list_rdma_info()
    ibmi_util.log_debug("List RDMA resources infomation result is: {0}".format(result), module._name)
    ibmi_util.log_debug("RDMA resources infomation are: {0}".format(resoure_info), module._name)

    if rc:
        module.fail_json(rc=255, msg="Error when list RDMA resources infomation", job_log=result)

    module.exit_json(
        rc=SUCCESS, resource_info=resoure_info)
