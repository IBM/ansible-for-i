...
options:
  ...
  wait:
    description:
      - If delivery_format is C(*SAVF), and C(wait) set to C(true), moudle will wait until all PTF save files are delivered or
        time is up.
      - If delivery_format is C(*IMAGE), module will return after the order is sent.
    type: bool
    default: True
  parameters:
    description:
...
problem_id:
    description: The problem identifier of the PTF order.
    returned: always
    type: int
    sample: 2021278656
msg:
...
def remove_pending_joblog(job_name, user_name, job_number):
    conn1 = dbi.connect()
    itransport = DatabaseTransport(conn1)
    itool = iToolKit()
    itool.add(
    ...
    ibmi_util.itoolkti_close_connection(conn1)
...
def return_error(module, error, out, msg, job_log, rc, job_submitted_split, wait, delivery_format, result):
    if (job_submitted_split and job_log) and (wait is True or delivery_format == '*IMAGE'):
        ret, message = remove_pending_joblog(job_submitted_split[2], job_submitted_split[1], job_submitted_split[0])
        if ret != 0:
            error = error + '/n remove pending joblog fail./n' + message
...
def main():
    ...
    if rc != ibmi_util.IBMi_COMMAND_RC_SUCCESS:
        return_error(module, error, out, 'Submit job failed.', [], ibmi_util.IBMi_COMMAND_RC_ERROR, job_submitted_split, wait,
    ...
    try:
        ...
        if wait or delivery_format == '*IMAGE':
            rc, out, error = ibmi_util.itoolkit_run_sql(conn, sql_get_job_info)
        ...
        if time_up is True:
            return_error(module, error, '', 'Time up when waiting for SNDPTFORD complete.', [], ibmi_util.IBMi_COMMAND_RC_ERROR,
                         job_submitted_split, wait, delivery_format, result)

        if rc == ibmi_util.IBMi_COMMAND_RC_SUCCESS:
            job_log = db2i_tools.get_job_log(conn, job_submitted, startd)
        else:
            return_error(module, error, out, 'itoolkit_run_sql failed', [], ibmi_util.IBMi_COMMAND_RC_ERROR, job_submitted_split,
        ...
        if job_log:
                ...
                while problem_id == 0:
                    for i in range(len(job_log) - 1, -1, -1):
                        if job_log[i]['MESSAGE_ID'] == 'CPZ8C38':
                            problem_id = job_log[i]['MESSAGE_TEXT'][18:28]
            ...
            return_error(module, error, out, 'No joblog returned.', job_log, ibmi_util.IBMi_COMMAND_RC_ERROR,
                         job_submitted_split, wait, delivery_format, result)
        ...
        if delivery_format == '*IMAGE':
            if image_directory == '*DFT':
                file_path = '/QIBM/UserData/OS/Service/ECS/PTF/' + str(problem_id)
            else:
        ...
        result.update({'job_log': job_log if joblog or rc or success is False else [], 'stdout': '', 'stderr': error,
                       'download_list': download_list, 'rc': rc, 'delta': str(delta), 'order_id': problem_id,
                       'msg': 'SNDPTFORD successfully end.', 'job_info': job_submitted, 'command': cl_sbmjob,
                       'order_start_time': order_start_time, 'order_end_time': order_end_time, 'file_path': file_path})

        module.exit_json(**result)

    except ImportError as e_import:
        return_error(module, str(e_import), '', '', '', ibmi_util.IBMi_PACKAGES_NOT_FOUND, job_submitted_split, wait,
                     delivery_format, result)
    except Exception as e_db_connect:
        return_error(module, str(e_db_connect), '', '', '', ibmi_util.IBMi_DB_CONNECTION_ERROR, job_submitted_split, wait,
                     delivery_format, result)
    finally:
