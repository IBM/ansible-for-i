...
def get_job_log_NLS(imodule, connection_id, job_name, time=None):
    # retreive current job ccsid
    rc, out_rtv, error = imodule.itoolkit_run_rtv_command('RTVJOBA', {'CCSID': 'number', 'DFTCCSID': 'number'})
    if rc:
        cast_ccsid = 37
    else:
        cast_ccsid = out_rtv['CCSID']
        if cast_ccsid == 65535:
            cast_ccsid = out_rtv['DFTCCSID']
    ...
    if (out_result_set is None) and (err is None):
        ...
        for result in out_result_set:
            result_map = {"ORDINAL_POSITION": result[0],
                          ...
                          "MESSAGE_FILE": result[17],
                          "MESSAGE_LIBRARY": result[18],
                          "MESSAGE_TOKENS": result[19],
                          "MESSAGE_TEXT": result[20],
                          "MESSAGE_SECOND_LEVEL_TEXT": result[21],
                          }
            out.append(result_map)
    return out
